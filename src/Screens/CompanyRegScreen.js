...
const FormStep1 = ({ nextStep }) => {
  ...
  const isFormComplete = () => {
    return name !== "" && email !== "" && phoneNo !== "" && password !== "" && cpassword !== "";
  };
    ...
    <div>
      ...
      <div className="row justify-content-center m-5">
        <div className="col-md-5 shadow mt-5">
          ...
          <button className="btn btn-success m-4" disabled={!isFormComplete()} onClick={nextStep}>
            Next
          </button>
        </div>
      </div>
...
const FormStep2 = ({ nextStep, prevStep }) => {
  ...
  const Register = async () => {
    ...
    try {
      setLoading(true);
      // const result = (await axios.post('/api/User/register-company', user)).data;
      setLoading(false);
      setSuccess(true);
      setCompanyName("");
      setCompanyAddress("");
      ...
      <div className="row justify-content-center m-5">
        <div className="col-md-5 shadow mt-5">
          ...
          <input
            type="number"
            className="form-control"
            placeholder="Enter Company Total Vehicles"
            value={companyTVehicle}
            onChange={(e) => {
              setCompanyTVehicle(e.target.value);
            }}
            required
          ></input>
          <br />
          <input
            ...
            value={companyTEmployee}
            onChange={(e) => {
              setCompanyTEmployee(e.target.value);
            }}
            required
          />
          <br />
          ...
          <button className="btn btn-primary m-3" onClick={prevStep}>
            Previous
          </button>
          <button className="btn btn-primary m-3" onClick={nextStep}>
            Next
          </button>
        </div>
      </div>
...
const CompanyRegScreen = () => {
  ...
  const renderFormStep = () => {
    switch (currentStep) {
      ...
      case 3:
        return <FormStep3 setFormData={setFormData} prevStep={prevStep} />;
      default:
        return null;
    }
  };
